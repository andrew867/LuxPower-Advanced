...
Then create a "Date and or time" helper for each of following, paste each of these names into the Name field and remember click on "Date and time" radio button at the bottom( helpers need to be exactly the same suffix as the number sensors generated by the integration) :
